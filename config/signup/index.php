...
function checkFormSignUp($name, $email, $username, $password, $cfpassword, $agree, $db)
{
    ...
    if ($white_letter != "") {
        echo showSnack("{$white_letter} must not contain spaces before or after", false); return false;
    }
    ...
    if ($special_chars != "") {
        echo showSnack("{$special_chars} cannot contain special characters", false); return false;
    }

    // check fullname have firstname and lastname
    if (check_contain_chars($name, [' '])) {
        echo showSnack("Full name must contain full first and last name", false); return false;
    }
    ...
    if ($number_char != "") {
        echo showSnack("{$number_char} must contain more than 6 characters", false); return false;
    }


    // check password and cf password must match
    if ($password != $cfpassword) {
        echo showSnack("password and confirm password must match", false); return false;
    }
    ...
    if ($result->num_rows > 0) {
        echo showSnack("Username already exists", false); return false;
    }
    ...
    $result = $db->query($sql);
    if ($result->num_rows > 0) {
        echo showSnack("Email already exists", false); return false;
    }
